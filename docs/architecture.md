...
| 模块 | 职责 |
| --- | --- |
| `movement.ts` | 单格移动、可走地砖检查、边界检查、碰撞、锁、方向石、传送带通行和传送带连续位移 |
...
可移动区域由 tilemap 中的深色石砖 tile ID `7-11` 定义，草地和装饰地面 tile ID `0-6` 只负责视觉背景，不允许 Bobby 进入。栅栏、锁、方向石、传送带等实体规则在可走地砖检查之后继续叠加。
