# Bobby Carrot Technical Architecture

Language: [中文](architecture.md) | English

This document describes the current Bobby Carrot codebase, including system boundaries, module responsibilities, core data flow, and extension points.

## 1. Product Scope

Bobby Carrot is a grid-based puzzle game built with Vite and TypeScript. Built-in levels are maintained as `LevelDefinition` TypeScript modules, and the game rules are centralized in a state machine and simulator.
...
| `docs/` | Project documentation, including this architecture document |
...
The Vite web build uses `base: /`. During GitHub Pages builds, `dist/index.html` is copied to `dist/404.html` and `dist/editor/index.html` so static hosting can handle the app routes.
...
| Built-in level data and asset manifest are large | Manual edits can miss fields or break structure | Run level validation after changes and prefer editor-exported level data when possible |
...
- Web entry: `src/main.ts`
- Browser runtime: `src/game/runtime.ts`
- Simulator entry: `src/game/simulation.ts`
- Movement rules: `src/game/movement.ts`
- Arrival interactions: `src/game/interactions.ts`
- Button system: `src/game/buttons.ts`
- Level definition: `src/game/levelDefinition.ts`
- State generation: `src/game/stateFactory.ts`
- Level validation: `src/game/levelValidation.ts`
- Canvas renderer: `src/game/render.ts`
- Sprite selection and drawing: `src/game/sprites.ts`
- Community level storage: `src/game/communityLevel.ts`
- Editor: `src/editor/app.ts`
- Editor model: `src/editor/editorModel.ts`
- TUI: `src/tui/cli.ts`
- Validation CLI: `src/tools/validate-levels.ts`
